...
class PyTrajFluxes_deriv:
    ...
    def __init__(self, epsilon, a, migr_param=0.0):
    ...
    def __call__(self, t, y):
        ...
        dJdt0, dJdt1, dJdt2 = np.real(fluxes(self.a, y[0], y[1], y[2]))
        # epsilon absorbed in the time derivative t = t * epsilon
        # Edot, Ldot, Qdot
        #dJdt[0],dJdt[1],dJdt[2] = dJdt0, dJdt1, dJdt2
        p,e,Y = E_roots_numba(self.a, y[0], y[1], y[2])

        x_new = Y_to_xI(self.a, p, e, Y)

        # effects of migration
        dJdt[0],dJdt[1],dJdt[2] = dJdt0, dJdt1, dJdt2
        Om1,Om2,Om3 = get_fundamental_frequencies(self.a, p, e, x_new)
...
class PyKerrGenericFluxInspiral(TrajectoryBase):
    ...
    def get_inspiral(self, M, mu, a, p0, e0, Y0, T=1.0, Phi_phi0=0.0, Phi_theta0=0.0, Phi_r0=0.0, **kwargs):
        ...
        def term_f(t,y):
            p,e,Y = E_roots_numba(a,y[0],y[1], y[2])
            x_new = Y_to_xI(a, p, e, Y)
            p_sep = get_separatrix(a, e, x_new)
            #print((p - p_sep)/p_sep -.1)

            return (p - p_sep)/p_sep -.1 # this defines the threshold
        ...
        # orbital elements
        p_e_Y = np.array([E_roots_numba(a, En[i], Lz[i], Q[i]) for i in range(len(En))])
