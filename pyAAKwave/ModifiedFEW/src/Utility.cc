                              ...
                              double a, double p, double e, double x)
{
    ...
    if ((e==0.0) && (x == 1.0))
    {
        *OmegaR_ = (Sqrt(2*a + (-3 + p)*Sqrt(p))*Sqrt((p*(-2*Power(a,2) + 6*a*Sqrt(p) + (-5 + p)*p + (Power(a - Sqrt(p),2)*(Power(a,2) - 4*a*Sqrt(p) - (-4 + p)*p))/abs(Power(a,2) - 4*a*Sqrt(p) - (-4 + p)*p)))/(2*a*Sqrt(p) + (-3 + p)*p)))/(Power(p,1.25)*(a + Power(p,1.5)));
        *OmegaTheta_ = (Sqrt(2*a + (-3 + p)*Sqrt(p))*abs((Power(p,0.25)*Sqrt(3*Power(a,2) - 4*a*Sqrt(p) + Power(p,2)))/Sqrt(2*a + (-3 + p)*Sqrt(p))))/(Power(p,1.25)*(a + Power(p,1.5)));
        *OmegaPhi_ = 1/(a + Power(p,1.5));
    }
    else
    {
...
double get_separatrix(double a, double e, double x)
{
    ...
    if ((e==0.0) && (x == 1.0))
    {
        double Z1 = 1. + (Power(1. + a, 1./3.) + Power(1. - a, 1./3.)) * Power((1. + a)*(1. - a), 1./3.);
        double Z2 = Sqrt(3.*a*a + Z1*Z1);
        p_sep = (3. + Z2 - Sqrt((3. - Z1)*(3. + Z1 + 2.*Z2)));
    }
    else
    {
