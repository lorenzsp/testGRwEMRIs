...
double KerrGeoEnergy(double a, double p, double e, double x)
{
	...
	return sqrt((Kappa * Rho + 2 * Epsilon * Sigma - x * 2 * sqrt(Sigma * (Sigma * (Epsilon*Epsilon) + Rho * Epsilon * Kappa - Eta * pow(Kappa,2))/(x*x)))/((Rho*Rho) + 4 * Eta * Sigma));
}
...
void KerrGeoRadialRootsLong(long double* r1_, long double*r2_, long double* r3_, long double* r4_, long double a, long double p, long double e, long double x, long double En, long double Q)
{
    long double M = 1.0;
    long double r1 = p / (1 - e);
    long double r2 = p /(1+e);
    long double AplusB = (2 * M)/(1-(En*En)) - (r1 + r2);
    long double AB = ((a*a) * Q)/((1-(En*En)) * r1 *  r2);
    long double r3 = (AplusB+sqrt((AplusB*AplusB) - 4 * AB))/2;
    long double r4 = AB/r3;
                              ...
                              double a, double p, double e, double x)
{
    ...
    // (*Eq. (21)*)
    double CapitalUpsilonPhi = (2 * CapitalUpsilonTheta)/(M_PI * sqrt(Epsilon0zp)) * EllipticPi(zm, (kTheta*kTheta)) + (2 * a * CapitalUpsilonr)/(M_PI * (rp - rm) * sqrt((1 - (En*En)) * (r1 - r3) * (r2 - r4))) * ((2 * M * En * rp - a * L)/(r3 - rp) * (EllipticK((kr*kr)) - (r2 - r3)/(r2 - rp) * EllipticPi(hp, (kr*kr))) - (2 * M * En * rm - a * L)/(r3 - rm) * (EllipticK((kr*kr)) - (r2 - r3)/(r2 - rm) * EllipticPi(hm, pow(kr,2))));
                              ...
                              double a, double p, double e, double x)
{
    ...
    double partA = (2 * M * En * rp - a * L) * (EllipticK((kr*kr)) - (r2 - r3)/(r2 - rp) * EllipticPi(hp, (kr*kr)));
    // addition to avoid dividing by zero by zero
    if (partA!=0.0){partA=partA/(r3-rp);}
    double partB = (2 * M * En * rm - a * L)/(r3 - rm) * (EllipticK((kr*kr)) - (r2 - r3)/(r2 - rm) * EllipticPi(hm, pow(kr,2)));

    double CapitalUpsilonPhi = (CapitalUpsilonTheta)/(sqrt(Epsilon0zp)) + (2 * a * CapitalUpsilonr)/(M_PI * (rp - rm) * sqrt((1 - (En*En)) * (r1 - r3) * (r2 - r4))) * ( partA - partB );

    // addition to avoid dividing by zero by zero
    double partC = ((4 * (M*M) * En - a * L) * rp - 2 * M * (a*a) * En) * (EllipticK((kr*kr)) - (r2 - r3)/(r2 - rp) * EllipticPi(hp, (kr*kr)));
    if (partC!=0.0){partC=partC/(r3-rp);}
    double partD = ((4 * (M*M) * En - a * L) * rm - 2 * M * (a*a) * En)/(r3 - rm) * (EllipticK((kr*kr)) - (r2 - r3)/(r2 - rm) * EllipticPi(hm,(kr*kr)));
    double part_tot = ( partC - partD);

    double CapitalGamma = 4 * (M*M) * En+ (2 * CapitalUpsilonr)/(M_PI * sqrt((1 - (En*En)) * (r1 - r3) * (r2 - r4))) * (En/2 * ((r3 * (r1 + r2 + r3) - r1 * r2) * EllipticK((kr*kr)) + (r2 - r3) * (r1 + r2 + r3 + r4) * EllipticPi(hr,(kr*kr)) + (r1 - r3) * (r2 - r4) * EllipticE((kr*kr))) + 2 * M * En * (r3 * EllipticK((kr*kr)) + (r2 - r3) * EllipticPi(hr,(kr*kr))) + (2* M)/(rp - rm) * part_tot );
