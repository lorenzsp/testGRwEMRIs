...
traj = EMRIInspiral(func="BeyondKerrCircFlux")
...
ndim_full = 18  # full dimensionality of inputs to waveform model

# which of the injection parameters you actually want to sample over
#                    M, mu, a, p0, dist, phi_phi0
test_inds = np.array([0, 1, 2, 3,                   14, 15])#, 6, 7, 8, 9, 11, 10, 12, 13])
...
priors_in = {0: uniform_dist(injection_params[test_inds[0]]*(1-perc), injection_params[test_inds[0]]*(1+perc)), 
            ...
            #  4: uniform_dist(0.0, 2.0*np.pi),
             5: uniform_dist(0.0,0.5),
             6: uniform_dist(0.0,0.5),
...
inv_gamma = np.diag(np.ones_like(injection_params[test_inds]))
...
for i in range(ndim):
    ...
    # else:
    start_points[:, i] = np.random.multivariate_normal(injection_params[test_inds], inv_gamma/5e14,size=nwalkers * ntemps)[:,i] 
    print('variable ',i)
...
sampler = PTEmceeSampler(
    ...
    plot_source="emri",
#    periodic=periodic,
    fp="search_beyond_snr_{:d}_no_noise_M{}_mu{}_a{}_p0{}_phi{}_d{}_Aacc{}_n{}_m{}_T{}.h5".format(
        int(snr_goal), M, mu, a, p0, Phi_phi0, scalar_charge, A_acc, n_acc, m_acc, T
    ),
