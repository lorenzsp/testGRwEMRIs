# python intr_mcmc.py -Tobs 2 -dt 15.0 -M 5e5 -mu 5.0 -a 0.95 -p0 13.0 -e0 0.4 -x0 1.0 -charge 0.0 -dev 3 -nwalkers 8 -ntemps 1 -nsteps 10
import argparse
import os
...
def run_emri_pe(
    ...
    moves = [
        (GaussianMove({"emri": cov}, mode="AM", factor=100, swap_walkers=None),0.7),
        (GaussianMove({"emri": cov}, mode="Gaussian", factor=100, swap_walkers=None),0.3),
    ]
...
if __name__ == "__main__":
    ...
    if logprior:
        fp = folder+ f"/MCMC_intr_M{M:.2}_mu{mu:.2}_a{a:.2}_p{p0:.2}_e{e0:.2}_x{x0:.2}_charge{charge}_SNR{source_SNR}_T{Tobs}_seed{SEED}_nw{nwalkers}_nt{ntemps}_logprior.h5"
    else:
        fp = folder + f"/MCMC_intr_M{M:.2}_mu{mu:.2}_a{a:.2}_p{p0:.2}_e{e0:.2}_x{x0:.2}_charge{charge}_SNR{source_SNR}_T{Tobs}_seed{SEED}_nw{nwalkers}_nt{ntemps}.h5"
